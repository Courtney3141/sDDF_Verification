...
void rx_return(void)
{
    ...
    while (reprocess) {
        while (!ring_empty(state.rx_ring_drv.used_ring)) {
            ...
            err = seL4_ARM_VSpace_Invalidate_Data(3, buffer.phys_or_offset + buffer_data_vaddr, buffer.phys_or_offset + buffer_data_vaddr + buffer.len);
            if (err) printf("MUX_RX|ERROR: ARM Vspace invalidate failed with err %d\n", err);
            assert(!err);

            int client = get_client((struct eth_hdr *) (buffer.phys_or_offset + buffer_data_vaddr));
            if (client >= 0) {
...
void init(void)
{
    mac_addr_init_sys(sel4cp_name, (uint8_t *) state.mac_addrs);

    ring_init(&state.rx_ring_drv, (ring_buffer_t *)rx_free_drv, (ring_buffer_t *)rx_used_drv, RX_RING_SIZE_DRIV);
    mux_ring_init_sys(sel4cp_name, state.rx_ring_clients, rx_free_arp, rx_used_arp);
